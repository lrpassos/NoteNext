  ...
  function renderNoteCard(note: KeepNote) {
    return (
      <div
        ...
        className="rounded-xl border border-gray-150 p-4 shadow-[0_3px_10px_rgba(0,0,0,0.01)] hover:shadow-md transition-shadow group flex flex-col justify-between relative"
      >
        
        {/* Dynamic header / pin button */}
        <div>
          <div className="flex justify-between items-start mb-2">
            <h4 className="text-xs font-bold text-gray-900 pr-4 leading-normal font-display">
              {note.title}
            </h4>
          ...
          <p className="text-xs text-gray-650 font-sans leading-relaxed whitespace-pre-line mb-3">
            {note.content}
          </p>
            ...
            <div className="space-y-1.5 border-t border-gray-100/40 pt-2 mb-3">
              {note.checklist.map((item) => (
                ...
                >
                  ...
                  <span className={`leading-tight ${item.checked ? "line-through text-gray-400 italic" : "text-gray-700"}`}>
                    {item.text}
                  </span>
                </div>
              ))}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100/50 text-[10px] mt-auto">
          ...
          <div className="flex flex-wrap gap-1">
            {note.tags.map((tag) => (
              <span 
                key={tag} 
                className="bg-white/70 border border-gray-150 px-1.5 py-0.5 rounded text-[9px] text-gray-500 font-semibold flex items-center gap-0.5"
              >
                <Tag className="w-2 h-2 text-brand-600" />
                <span>{tag}</span>
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="flex items-center gap-0.5 bg-white/70 px-1 py-0.5 rounded border border-gray-150 mr-1">
              {["#ffffff", "#dcfce7", "#eff6ff", "#fffbeb"].map((col) => (
                <button
                  key={col}
                  onClick={() => changeNoteColor(note.id, col)}
                  style={{ backgroundColor: col }}
                  className="w-2.5 h-2.5 rounded-full border border-gray-300 hover:scale-110"
                />
              ))}
            </div>
            ...
            <button
              onClick={() => onTriggerAi(`Resuma as notas e crie uma ação prática sobre este tema: ${note.title} - ${note.content} ${note.checklist.map(c=>c.text).join(",")}`, note.id)}
            ...
            <button
              ...
              className="w-5.5 h-5.5 rounded-md hover:bg-red-50 text-gray-450 hover:text-red-650 flex items-center justify-center transition-colors"
              title="Deletar nota"
            >
        ...
        <div className="text-[8px] text-gray-400 mt-2 block select-none">Atividades: {note.updatedAt}</div>
      </div>
    );
  }
}
